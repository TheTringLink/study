document.addEventListener('DOMContentLoaded', function() {
    ...
    if (infoModule && infoToggleBtn) {
        // 1. 自動偵測螢幕寬度 (判定是否為手機)
        function checkDeviceSize() {
            // 768px 通常是手機與平板的斷點
            if (window.innerWidth <= 768) {
                // 手機：預設加上 collapsed 類別 (縮小)
                infoModule.classList.add('collapsed');
            } else {
                // 電腦：移除 collapsed 類別 (展開)
                infoModule.classList.remove('collapsed');
            }
        }
        ...
        infoToggleBtn.addEventListener('click', function() {
            infoModule.classList.toggle('collapsed');
        });
    }
